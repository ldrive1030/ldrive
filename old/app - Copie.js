(function() {
    ...
    function initMap() {
        ...
        pickupMarker = L.marker([48.8566, 2.3522], { draggable: true }).addTo(map);
        dropoffMarker = L.marker([48.8566, 2.3522 + 0.02], { draggable: true }).addTo(map);
            ...
            if (driverBtn) {
                driverBtn.addEventListener('click', async () => {
                    await db.collection('users').doc(user.uid).set({
                        ...
                        phone: '',
                        role: 'driver',
                        licenseVerified: true  // on valide directement sans upload
                    });
                    auth.onAuthStateChanged(() => {});
                });
            }
    ...
    function showRoleChoice() {
        ...
        main.innerHTML = `
            <div class="role-choice-container">
                ...
                <div id="auth-forms-container" style="margin-top: 20px;"></div>
            </div>
        `;
        ...
        if (clientBtn) {
            clientBtn.addEventListener('click', () => {
                authContainer.innerHTML = getAuthFormHTML('client');
                attachAuthListeners('client');
            });
        }
        if (driverBtn) {
            driverBtn.addEventListener('click', () => {
                authContainer.innerHTML = getAuthFormHTML('driver');
                attachAuthListeners('driver');
            });
        }
    }

    function getAuthFormHTML(role) {
        if (role === 'client') {
            return `
                <div class="auth-form">
                    <h3>Inscription / Connexion cliente</h3>
                    <button id="google-auth-btn" class="social-btn">🔐 Continuer avec Google</button>
                    <div style="text-align: center; margin: 12px 0;">ou</div>
                    ...
                    <p style="text-align:center; margin-top:12px;">Déjà un compte ? <span id="switch-to-login" style="color:#007aff; cursor:pointer;">Connectez-vous</span></p>
            ...
            return `
                <div class="auth-form">
                    <h3>Inscription / Connexion conductrice</h3>
                    <button id="driver-google-auth-btn" class="social-btn">🔐 Continuer avec Google</button>
                    <div style="text-align: center; margin: 12px 0;">ou</div>
                    <input type="text" id="driver-name" placeholder="Nom complet">
                    <input type="email" id="driver-email" placeholder="Email">
                    <input type="tel" id="driver-phone" placeholder="Téléphone">
                    <input type="password" id="driver-password" placeholder="Mot de passe">
                    <!-- pas de champ photo -->
                    <button id="driver-signup-btn">Créer un compte</button>
                    <p style="text-align:center; margin-top:12px;">Déjà inscrite ? <span id="driver-switch-to-login" style="color:#007aff; cursor:pointer;">Connectez-vous</span></p>
                    <div id="driver-login-form" style="display: none;">
                        <hr>
                        <input type="email" id="driver-login-email" placeholder="Email">
                        <input type="password" id="driver-login-password" placeholder="Mot de passe">
                        <button id="driver-login-btn">Se connecter</button>
                    </div>
                </div>
            `;
        }
    ...
    function attachAuthListeners(role) {
        if (role === 'client') {
            ...
            // Driver auth
            const googleBtn = document.getElementById('driver-google-auth-btn');
            if (googleBtn) {
                googleBtn.addEventListener('click', async () => {
                    const provider = new firebase.auth.GoogleAuthProvider();
                    try {
                        ...
                        if (!doc.exists) {
                            ...
                            showToast('Compte conductrice créé !');
                        } else {
                            const existingRole = doc.data().role;
            ...
            const signupBtn = document.getElementById('driver-signup-btn');
            if (signupBtn) {
                signupBtn.addEventListener('click', async () => {
                    ...
                    try {
                        ...
                        showToast('Compte conductrice créé avec succès !');
                    } catch (error) {
                        console.error('Erreur inscription conductrice:', error);
                        showToast('Erreur : ' + error.message);
                    } finally {
                        hideLoading();
                    }
            ...
            const switchToLogin = document.getElementById('driver-switch-to-login');
    ...
    function displayClientTracking(ride, rideId) {
        ...
        L.marker([ride.pickupCoords.lat, ride.pickupCoords.lng]).addTo(trackingMap).bindPopup('Prise en charge');
        L.marker([ride.dropoffCoords.lat, ride.dropoffCoords.lng]).addTo(trackingMap).bindPopup('Destination');
    ...
    async function listenForPendingRides() {
        ...
        // Pas de vérification de permis car désactivé
        const q = db.collection('rides')
    ...
    function displayPendingRides(pendingList) {
        ...
        pendingList.forEach(ride => {
            ...
            if (mapContainer) {
                ...
                L.marker([ride.pickupCoords.lat, ride.pickupCoords.lng]).addTo(miniMap).bindPopup('Départ');
                L.marker([ride.dropoffCoords.lat, ride.dropoffCoords.lng]).addTo(miniMap).bindPopup('Destination');
    ...
    function displayDriverActiveRide(ride, rideId) {
        ...
        L.marker([ride.pickupCoords.lat, ride.pickupCoords.lng]).addTo(driverTrackingMap).bindPopup('Prise en charge');
        L.marker([ride.dropoffCoords.lat, ride.dropoffCoords.lng]).addTo(driverTrackingMap).bindPopup('Destination');
